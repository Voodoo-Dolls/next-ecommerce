    ...
    return (
        ...
        >
            ...
            >
                {slides.map((slide) => {
                    return (
                        <div className={`${slide.bg} w-screen xl:h-full flex flex-col gap-16 xl:flex-row`} key={slide.id}>
                            {/* TEXT CONTIANER */}
                            <div className="xl:w-1/2 h-1/2 flex items-center justify-center gap-8 flex-col 2xl:gap-12 text-center xl:h-full">
                                <h2 className="text-xl lg:text-3xl 2xl:text-5xl ">{slide.description}</h2>
                                <h1 className="text-5xl lg:text-6x; 2xl:text-8xl font-semibold">{slide.title}</h1>
                                <Link href={slide.url}>
            ...
            <div className="absolute m-auto left-1/2 bottom-8 gap-4 flex">
                {slides.map((slide, index) => (
                    ...
                    >
                        {current === index && (<div className="bg-white rounded-full w-[6px] h-[6px]" />)}
                    </div>
                ))}
            </div>
